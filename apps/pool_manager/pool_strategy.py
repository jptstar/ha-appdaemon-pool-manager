...
class StrategyMixin:
    ...
    def calcule_vitesse_garantie_quota(self, filtre_temps_eq, objectif_temps_eq):
        ...
        vitesse_necessaire = int(round((reste_temps_eq / temps_restant_h) * vitesse_ref))
    ...
    def quota_en_retard_obligatoire(self, filtre_temps_eq, objectif_temps_eq):
        ...
        vitesse_ref = max(self.vitesse_min_filtration_utile, self.vitesse_reference_filtration)
        temps_possible_eq = temps_restant_h * (self.vitesse_max_garantie_quota / vitesse_ref)

        if reste_temps_eq >= temps_possible_eq * 0.95:
            return True, vitesse

        if vitesse > self.vitesse_min_filtration_utile:
            return True, vitesse

        return False, vitesse
    ...
    def appliquer_priorite_pac_ou_quota(self, filtre_temps_eq, objectif_temps_eq, pompe_on, pac_besoin, pac_chauffe, etat_pac, surplus_net=None, reseau_net=None, pv_power=None, extra=""):
        quota_obligatoire, vitesse_quota = self.quota_en_retard_obligatoire(filtre_temps_eq, objectif_temps_eq)
        ...
        if self.pac_prioritaire_absolue and pac_besoin:
            ...
            vitesse = max(self.vitesse_min_filtration_utile, self.vitesse_pac_prioritaire, self.vitesse_min_pac_active)
            if quota_obligatoire and vitesse_quota is not None:
                vitesse = max(vitesse, vitesse_quota)

            libelle = "PAC prioritaire + quota" if quota_obligatoire else "PAC prioritaire"

            if not pompe_on:
                self.start_pump_with_delayed_speed(vitesse, delay_s=2, context="pac_prioritaire")
                self.set_messages(
                    f"{libelle} | {filtre_temps_eq:.1f}/{objectif_temps_eq:.1f}h",
                    f"démarrage | {vitesse}% | {filtre_temps_eq:.1f}/{objectif_temps_eq:.1f}h"
                )
                self.set_debug_w(f"PAC {etat_pac}")
                return True

            vitesse_appliquee = self.set_pump_percentage(vitesse)
            short_msg, detail_msg = self.split_message_solaire(libelle, vitesse_appliquee, filtre_temps_eq, objectif_temps_eq, extra=extra)
            self.set_messages(short_msg, detail_msg)
        ...
        if quota_obligatoire and vitesse_quota is not None:
            ...
            if not pompe_on:
                self.start_pump_with_delayed_speed(vitesse, delay_s=2, context="garantie_quota")
                self.set_messages(
                    f"Garantie quota | {filtre_temps_eq:.1f}/{objectif_temps_eq:.1f}h",
